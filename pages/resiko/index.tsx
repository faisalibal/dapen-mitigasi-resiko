...
import { DepartmentDTO, ResikoInherentDTO } from '@/DTO/RiskDTO';
...
const ChartInherent = dynamic(
  () => import('@/components/molecul/ChartInherent'),
  { ssr: false }
);
...
const Resiko = () => {
  ...
  const [dataInherent, setDataInherent] = useState<ResikoInherentDTO[]>();
  const [dataRisiko, setDataRisiko] = useState<ResikoInherentDTO[]>();
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (indexToUpdate !== undefined) {
        ...
        if (prevData) {
          ...
          if (dataColumn) {
            newData[indexToUpdate] = {
              ...newData[indexToUpdate],
              PROBABILITAS_INHERENT: dataColumn?.probabilitas,
              DAMPAK_INHERENT: dataColumn?.dampak,
              NILAI_INHERENT: dataColumn?.nilai,
            };
          }
          return newData;
        }
      ...
      try {
        ...
        data.append('dampak_inherent', dataColumn?.dampak ?? '');
        data.append('prob_inherent', dataColumn?.probabilitas ?? '');
        data.append('level_of_risk_inherent', dataColumn?.nilai ?? '');
        data.append('line_no', draggableId);

        const response = await baseURL.put('/level_of_risk_inherent', data, {
          headers: {
